...
def create_timeline_view(df_filtered):
    ...
    for _, row in df_sorted.iterrows():
        ...
        # RAG status emoji
        rag_emoji = {"Green": "🟢", "Amber": "🟡", "Red": "🔴"}.get(row['rag_status'], "⚪")
        ...
        with st.container():
            ...
            with col3:
                st.markdown(f"{rag_emoji} {row['rag_status']}")
...
def main():
    ...
    with col2:
        # RAG Status
        rag_summary = df['rag_status'].value_counts()
        st.write("**RAG Status:**")
    ...
    if not df_filtered.empty:
        ...
        display_df = df_filtered[['ticket_id', 'summary', 'status', 'rag_status', 'start_date', 'due_date', 'hierarchy_level']].copy()
